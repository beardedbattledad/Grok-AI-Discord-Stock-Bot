...
# ====================== YOUR TRADING RULES (strict for alerts only) ======================
TRADING_RULES = """
MUST PASS ALL:
1. Aggressive execution: Sweep or at/above ask (calls) or at/below bid (puts).
2. New opening: Volume or contracts > current Open Interest.
3. No chasing: Today's move < |3%| (or 60-min < |2%|) unless Major Index ETF.
4. Premium meets tier (ETF ≥$1M, Mega ≥$500K, Mid ≥$100K, Small ≥$50K).
Only directional (bullish calls or bearish puts). Be extremely short.
"""

# ====================== DATA FETCHERS ======================
async def get_flow_alerts(min_premium=0, limit=200):
    try:
...
# ====================== AUTO ALERT SCANNER (now uses your custom filters) ======================
...
async def auto_alert_scanner():
    ...
    for f in CUSTOM_FILTERS:
        try:
            trades = await get_flow_alerts(min_premium=f["min_premium"], limit=200)
            for trade in trades:
                vol = trade.get("volume", 0)
                oi = trade.get("open_interest", 1)
                premium = trade.get("premium", 0)
                # Basic high-conviction check (will be expanded with full rules)
                if vol > oi * 3 and premium >= f["min_premium"]:
                    alert = format_short_alert(trade)
                    await channel.send(alert)
                    await asyncio.sleep(1.5)  # avoid spam
        except Exception as e:
    ...
    if bot.user.mentioned_in(message) or isinstance(message.channel, discord.DMChannel):
        ...
        try:
            ...
            if ticker:
                ...
                if ticker_flow:
                    flow_data = ticker_flow + flow_data
